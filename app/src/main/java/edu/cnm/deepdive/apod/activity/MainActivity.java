...
public class MainActivity extends AppCompatActivity {
  ...
  protected void onSaveInstanceState(Bundle outState) {
    ...
    outState.putLong(CALENDER_KEY, calendar.getTimeInMillis());
  ...
  private void setupDefault(Bundle savedInstanceState) {
    ...
    if (savedInstanceState != null) {
      calendar.setTimeInMillis(savedInstanceState.getLong(CALENDER_KEY));
  ...
  private class ApodTask extends AsyncTask<Date, Void, Apod> {
    ...
    protected void onCancelled(Apod apod) {
      ...
      progressSpinner.setVisibility(View.GONE);
      Toast.makeText(context, context.getString(R.string.error_message, date), Toast.LENGTH_LONG)
          .show();
    }
